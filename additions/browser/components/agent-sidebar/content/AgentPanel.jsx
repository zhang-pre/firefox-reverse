...
function AssistantBody({ steps, content }) {
  ...
  const collapsible =
    visibleSteps.some(s => s.kind !== "text") || visibleSteps.filter(s => s.kind === "text").length > 1;
  const shown = collapsed ? (lastTextIdx >= 0 ? [visibleSteps[lastTextIdx]] : visibleSteps.slice(-1)) : visibleSteps;
  return (
...
export default function AgentPanel({ buildClient, conversations, store, router, runAgentTurn, session, isVisionModel, workspace, notes, skill, toolNames = [], onOpenEnvironment, onOpenSettings, hidden = false }) {
  ...
  const [mode, setMode] = useState(null); // auto=全自动 / assist=用户领航 / null=未选
  ...
  async function newChat() {
    ...
    bindWorkspace(effectiveWorkspace(t));
    setMode(null); // 新会话未选模式 → 空状态里弹两种执行策略
